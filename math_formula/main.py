...
def mf_check(context: bpy.types.Context) -> bool:
    ...
    return (
        ...
        and space.tree_type in possible_trees
    )
...
class MF_OT_select_from_root(bpy.types.Operator):
    ...
    def select_parents_of(
        self,
        node: bpy.types.Node,
        links: bpy.types.NodeLinks,
        visited: list[bpy.types.Node],
    ) -> None:
        # Prevent loops
        if node in visited:
            return
...
class MF_OT_arrange_from_root(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context):
        ...
        node_positioner = TreePositioner(
            ...
            invert_relations=self.invert_relations,
        )
...
class MF_OT_math_formula_add(bpy.types.Operator, MFBase):
    ...
    def modal(self, context: bpy.types.Context, event: bpy.types.Event):
        ...
        links = space.edit_tree.links
        cursor_loc = (
            cast(tuple[int, int], space.cursor_location)
            if self.use_mouse_location
            else (0, 0)
        )
        node_positioner = TreePositioner(context)
        node_positioner.place_nodes(self.root_nodes[0], links, cursor_loc=cursor_loc)
        for i, tree in enumerate(self.node_group_trees):
            node_positioner = TreePositioner(context)
...
class MF_OT_type_formula_then_add_nodes(bpy.types.Operator, MFBase):
    ...
    def internal_error(self, remove_handle: bool = True) -> None:
        self.report({"ERROR"}, "Internal error, please report as a bug (see console)")
        print("\n\nERROR REPORT:\n")
        traceback.print_exc()
    ...
    def modal(self, context: bpy.types.Context, event: bpy.types.Event):
        ...
        # elif here since this was a key release and not a key press.
            ...
            if event.ctrl:
                ...
                compiler = Compiler(
                    cast(
                        str,
                        cast(bpy.types.SpaceNodeEditor, context.space_data).tree_type,
                    ),
        ...
        # Compile and check for errors
            ...
            compiler = Compiler(
                cast(
                    str, cast(bpy.types.SpaceNodeEditor, context.space_data).tree_type
                ),
                file_loading.file_data,
            )
            try:
        ...
        # NAVIGATION
            ...
            self.editor.pos = (
                self.old_editor_loc[0] + event.mouse_region_x - self.old_mouse_loc[0],
        ...
        # FORMULA HISTORY NAVIGATION
            ...
            self.editor.replace_text(formula_history[formula_history_index])
