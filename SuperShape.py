...
class MESH_OT_addSupershape(bpy.types.Operator, bpy_extras.object_utils.AddObjectHelper) :
    ...
    def execute(self, context) :
        ...
        for i in range (0, Unum + 1):
            phi = -math.pi/2
            #Superformula
            r1 = ((abs(math.cos(m*theta/4)/a))**n2+(abs(math.sin(m*theta/4)/b))**n3)**(-1/n1)
            for j in range(0,Vnum + 1):
                r2 = ((abs(math.cos(m*phi/4)/a))**n2+(abs(math.sin(m*phi/4)/b))**n3)**(-1/n1)
